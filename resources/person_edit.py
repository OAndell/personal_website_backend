    ...
    try:
       ...
       if(user[0]['password'] == hashlib.sha512(password.encode()+ user[0]['salt'].encode()).hexdigest()):
            ...
            execute('''
              ...
              WHERE id=%s ''',
              (
                newdataJson['name'], newdataJson['title'],
                newdataJson['email'], newdataJson['location'],
                newdataJson['resume'], newdataJson['image'],
                newdataJson['github'],newdataJson['linkedin'],
                newdataJson['id']
            ))
